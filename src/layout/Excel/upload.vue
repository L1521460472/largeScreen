<template>
  <div style="display: inline-block">
    <el-button :type="type || ''" icon="el-icon-upload2" v-waves @click="showUpload" :disabled="$hasPerm('import')">{{importBtnName || $t('table.import')}}</el-button>
    <el-dialog title="导入" :visible.sync="visible"  class="demo-box" width="40%">
          <el-upload
            class="upload-demo"
            ref="upload"
            action=""
...
<script>
  ...
  export default {
    ...
    props:{ type: {type:String,require:false}, importBtnName: {type:String,require:false}, templateName:{type:String,require:true},uploadUrl:{type:String,require: true}},
    methods: {
      showUpload(){
        this.visible=true;
      },
      ...
      submitUpload() {
        ...
        service.post(this.uploadUrl + (this.templateName ? "/"+this.templateName : ''), this.submitObj, config).then( (res) => {
