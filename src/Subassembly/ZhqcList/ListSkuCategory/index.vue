<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.categoryCode }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;margin-left:30px;">{{ item.categoryName }}</span>
        <!--<span style="float: left; color: #8492a6; font-size: 13px;margin-left:30px;">{{ item.origSysName }}</span>-->
      </slot>
    </el-option>
  </el-select>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/sku/skuCategory/queryCbList',
        itemUrl: process.env.WMS_MODEL+'/base/sku/skuCategory/queryCb/',
      };
    },
    methods: {
      /**
