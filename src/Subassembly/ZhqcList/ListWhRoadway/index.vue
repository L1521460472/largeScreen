<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.roadwayCode }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;margin-left:30px;">{{ item.roadwayName }}</span>
      </slot>
    </el-option>
  </el-select>
</template>
...
<script>
  ...
  export default {
    ...
    watch: {
          ...
          new Promise((resolve, reject) => {
            this.itemApi().then(res => {
              if (res.code === this.$successCode) {
                this.list = [
                  {
                    id: res.obj.entity.id,
                    roadwayCode: res.obj.entity.roadwayCode,
                    roadwayName: res.obj.entity.roadwayName,
                  }
                ];
      ...
      zoneId(val){
        if(this.whZoneId && this.whZoneId != val){
          this.value=null;
        }
        this.whZoneId=val;
        this.getList();
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/wh/whRoadway/queryWhRoadwayCbList',
        itemUrl: process.env.WMS_MODEL+'/base/wh/whRoadway/initUpdate/',
      };
    },
    methods: {
      /**
