<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.lotCode }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;margin-left:30px;">{{ item.lotName }}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/wh/whLot/queryWhLotCbList',
        itemUrl: process.env.WMS_MODEL+'/base/wh/whLot/initCbByCode/',
      };
    },
    methods: {
      /**
       ...
       */
      listApi(queryText) {
        let data = {queryText: queryText,lotType:this.lotType, page: this.$globalPage, limit: this.$globalLimit};
        return service({
          url: this.listUrl,
          method: 'post',
          data
      ...
      selectItem(value) {
        ...
        if(this.list && this.value) {
          for (let i = 0; i < this.list.length; i++) {
            if (this.list[i].lotCode == this.value) {
