...
const Validator = {
  ...
  getCardIDValidator() {
    ...
    let validCardID = (rule, value, callback) => {
      if (!value) {
  ...
  getQQValidator() {
    ...
    let validQQ = (rule, value, callback) => {
      if (!value) {
  ...
  getMoneynumValidator(){
    ...
    let validMoneynum = (rule, value, callback) => {
      value=value+"";
      if (!value) {
        callback(new Error('请输入数字'))
...
//
let newObj = {};
for(let i in moduleStores){
  Object.assign(newObj,moduleStores[i])
}

// console.log(bdRoutePrice)
