<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.zoneCode }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;margin-left:30px;">{{ item.zoneName }}</span>
      </slot>
    </el-option>
  </el-select>
</template>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      areaId(val){
        if(this.whArea && this.whArea != val){
          this.value=null;
        }
        this.whArea=val;
        this.getList();
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/wh/whZone/queryWhZoneCbList',
        itemUrl: process.env.WMS_MODEL+'/base/wh/whZone/initUpdate/',
      };
    },
    methods: {
      /**
