<template>
  <el-select ref="refSelect" v-model="value" placeholder="请选择市"
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      provinceId(val){
        if(this.province && this.province != val){
          this.value=null;
        }
        this.province=val;
        this.getList();
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/address/city/queryCityList/',
      };
    },
    methods: {
      /**
      ...
      listApi() {
        return service({
          url: this.listUrl+this.province,
          method: 'get',
        });
      },
      /**
