<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.areaCode }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;margin-left:30px;">{{ item.areaName }}</span>
      </slot>
    </el-option>
  </el-select>
</template>
  ...
  export default {
                ...
                this.list = [
                  {
                    ...
                    areaCode: res.obj.entity.areaCode,
                    areaName: res.obj.entity.areaName,
                  }
                ];
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/wh/whArea/queryWhAreaCbList',
        itemUrl: process.env.WMS_MODEL+'/base/wh/whArea/initUpdate/',
      };
    },
    methods: {
      /**
       ...
       */
      listApi(queryText) {
        let data = {areaCode: queryText, page: this.$globalPage, limit: this.$globalLimit};
        return service({
          url: this.listUrl,
          method: 'post',
          data
