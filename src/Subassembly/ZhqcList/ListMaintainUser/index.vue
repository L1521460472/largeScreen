<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px;width:120px">{{ item.maintainUser }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;width:120px">{{ item.maintainUserName }}</span>
        <span style="float: left; color: #8492a6; font-size: 13px;width:50px;">{{ item.maintainTypeName }}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/gsp/pro/maintainUser/queryMaintainUserCbList',
        itemUrl: process.env.WMS_MODEL+'/gsp/pro/maintainUser/initUpdate/',
      };
    },
    methods: {
      /**
       ...
       */
      listApi(queryText) {
        let data = {queryText: queryText, page: this.$globalPage, limit: this.$globalLimit};
        return service({
          url: this.listUrl,
          method: 'post',
          data
