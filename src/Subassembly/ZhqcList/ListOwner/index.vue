<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.ownerName }}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/partner/owner/queryOwnerCbList',
        itemUrl: process.env.WMS_MODEL+'/base/partner/owner/initUpdate/',
      };
    },
    methods: {
      /**
       ...
       */
      listApi(queryText) {
        let data = {ownerCode: queryText, page: this.$globalPage, limit: this.$globalLimit};
        return service({
          url: this.listUrl,
          method: 'post',
          data
