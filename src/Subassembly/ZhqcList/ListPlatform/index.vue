<template>
  <el-select ref="refSelect" v-model="value" placeholder="请输入" remote :remote-method="getList"
    ...
    >
      <slot>
        <span style="float: left; color: #8492a6; font-size: 13px">{{ item.platformName }}</span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        list: [],
        loading: false,
        listUrl: process.env.WMS_MODEL+'/base/tm/platform/queryPlatformCbList',
        itemUrl: process.env.WMS_MODEL+'/base/tm/platform/initUpdate/',
      };
    },
    methods: {
      /**
       ...
       */
      listApi(queryText) {
        let platformTypeList=['UNIVERSAL_PLATFORM'];
        platformTypeList.push(this.platformType);
        let data = {queryText: queryText,platformTypeList:platformTypeList, page: this.$globalPage, limit: this.$globalLimit};
        return service({
          url: this.listUrl,
          method: 'post',
          data
